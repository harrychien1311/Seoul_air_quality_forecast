...
def main():
    ...
    with col5:
        ...
        st.metric(label = "PM25", value = df.iloc[0]["pm25"])
        st.metric(label = "PM10", value = df.iloc[0]["pm10"])
        st.metric(label = "O3", value = df.iloc[0]["o3"])
        st.metric(label = "no2", value = df.iloc[0]["no2"])
        st.metric(label = "SO2", value = df.iloc[0]["so2"])
        st.metric(label = "CO", value = df.iloc[0]["co"])
